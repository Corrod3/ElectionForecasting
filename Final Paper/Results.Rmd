...
Our results show that the choice of weighting method has a considerable effect on the performance of the forecast against our benchmark data. Moreover, we could show that proper and transparent weighting with publicly available elections statistics and Census data is feasible to obtain a representative and reasonable forecast from the online poll. Table \ref{tab:results} displays the SZ' rolling average for March 2017 and December 2016, our weighted results and the raw polling data together with the calculated root mean squared error (RMSE) as indicator of accuracy of prediction.
...
As the table shows, the divergence of the raw data was relatively high compared to the average of official polls. The raw forecast has a RMSE of 5.83 for December and 4.75 in March. Yet, with our weighting approach using Census data we were able to improve this performance only by a negligible amount. Employing clusters of gender, age and religion our forecasts show a RMSE of 5.55 and 4.61 respectively. The weak performance might be due to several reasons. As stated earlier the Dalia data is stratified with age and gender by the time the poll in conducted. We decided, however, to account for age and gender through census data since we did not transparently evaluate Dalia's procedure for stratification. Yet, the very low difference of our estimate with Census data and the original data shows that our approach did not siginificantly differ from Dalia's approach. Moreover, the variable religion does not seem to have a reasonable impact on our weights. In sum, the approach using exclusively Census data does not account for likely voter bias at all.

In contrast, our approach using the election statistics does account for demographics of likely voter and thus performs very well compared to Census data. Weighting with cluster of age, gender and vote decision and taking into account self-reported vote at the past election yields results in RMSE values of 2.17 and 2.05 respectively compared to our benchmark. Figure \ref{fig:weightedplot} contrasts the raw data with our weighted estimates for December and March.
...
This shows, how close our data is to the benchmark as well as how it follows the huge shift in vote shares for the SPD after March 2017 - the so called "Schulz-Effect". Moreover, our estimates are lying within or close to the colored areas besides the rolling average which are displaying the confidence intervals. While that illustrates that they are often accurate there is still some considerable error, which leaves room for further improvement. This could be done either by using more detailed cluster, different approaches to account for likely voter bias or usage of different data sources such as exit polls of polling institutes.